...
class MerminExcitationHelper(CoreExcitationHelper):
    ...
    # Generic Mermin solver routine
    def Solver(self, Plan, tau=0, N0=None,
        ...
        if ForceSym:
            ...
            while k<Ne:
                ...
                for k1 in range(k+1,min(k+SMax,Ne)):
                    if np.abs(self.epsilon0[k1]-e0)<SymCut:
                        Groups[k] += [k1]
                k = np.max(Groups[k])+1

        if DIIS>0:
            NOrb = self.C0.shape[1]
            D_In  = np.zeros((DIIS,NOrb,NOrb))
            D_Out = np.zeros((DIIS,NOrb,NOrb))
        ...
        for step in range(MaxIter):
            if DIIS>0:
                # Handle DIIS
                # Note, D_In and D_Out are in the self.C0 basis
                E, FC, D_In[k_DIIS,:,:], D_Out[k_DIIS,:,:] \
                    = FullMerminEnergyFock(self, f, tau, C=C)
                k_DIIS += 1
                k_DIIS = k_DIIS % DIIS
            else:
            ...
            if (DIIS<=0) or not(k_DIIS==0):
                ...
                # Handle the DIIS
                # First get the weights on the densities
                dD = D_In-D_Out
                A = np.zeros((DIIS+1,DIIS+1))
                A[:DIIS,:DIIS] = np.tensordot(dD, dD, axes=((1,2),(1,2)))
                A[:DIIS,-1] = 1.
                A[-1,:DIIS] = 1.
                b = np.zeros((DIIS+1))
                b[-1] = 1.
                ...
                # Form the optimal density in self.C0 basis
                D = np.tensordot(c, D_In, axes=((0,),(0,)))
                # Diagonalise to get f and C = self.C0.dot(u)
                f, u = la.eigh(D)
                C = self.C0.dot(u)
