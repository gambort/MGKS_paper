...
class CoreExcitationHelper:
    ...
    # Find the highest orbital in each symmetry
    def GetAllH(self):
        SymAll = np.sort(np.unique(self.Sym_k))
        SymAll = [int(S) for S in SymAll]
        kh = {}
        for Sym in SymAll:
            for q in range(self.Engine.kh+1):
                k = self.Engine.kh - q
                if self.Sym_k[k] == Sym:
                    kh[Sym] = k
                    break
    ...
    # Find the lowest orbital in each symmetry
    def GetAllL(self):
        SymAll = np.sort(np.unique(self.Sym_k))
        SymAll = [int(S) for S in SymAll]
        kl = {}
        for Sym in SymAll:
            for k in range(self.Engine.kh+1, self.Engine.nbf):
                if self.Sym_k[k] == Sym:
                    kl[Sym] = k
                    break
    ...
    # Find all valid lowest transitions
    def GetAllTransitions(self, Ordered=False):
        """Get a list of symmetry-preserved lowest transitions, i.e. the
        frontier transition from symmetry Sf to symmetry St governed by
        indices (kf, kt)

        Args:
            Ordered (bool, optional): Provide an ordering based on frozen
                  orbital singlet energies Eft. Defaults to False.

        Returns:
            dict: Dictionary whose keys are (Sf, St) and values are (kf, kt)
                  if Ordered=False
            or
            list: Ordered list of (Sf, St, kf, kt, Eft)
                  if Ordered=True
        """
        ...
        self.ResetOrbitals()
        Plan = self.SolveGS(PlanOnly = True)
        E0 = self.SolverFrozen(Plan)
        A, B = [], []
        for Key in T:
            Sh, Sl = Key
            kh, kl = T[Key]
            Plan = self.SolveSX(kFrom = kh, kTo = kl, SwapOnly=True, PlanOnly=True)
            E = self.SolverFrozen(Plan)
            A += [E-E0,]
            B += [(Sh, Sl, kh, kl, E-E0)]

            if self.Report>=0:
                print("Sym = %2d -> %2d Orb = %3d -> %3d En = %6.2f eV"\
                    %(Sh, Sl, kh, kl, eV*(E-E0)), flush=True)

        kk = np.argsort(A)
        return [B[k] for k in kk]
    ...
    def SetFrom(self, k=None, Sym=None, **kwargs):
        ...
        if not(Sym is None):
            ...
            kh = self.GetAllH()
            if not(Sym in kh):
                print("No occupied orbital in that symmetry - setting HOMO")
    ...
    def SetTo(self, k=None, Sym=None, **kwargs):
        ...
        if not(Sym is None):
            ...
            kl = self.GetAllL()
            if not(Sym in kl):
                print("No virtual orbital in that symmetry - setting LUMO")
    ...
    def SolveSX(self, xi=None,
        ...
        if SwapOnly:
            Plan = PlanHandler(xi=np.abs(xi))\
                .Singlet(self.Nel, From=[self.kh], To=[self.kl])
            Plan.Swap(self.kh,kFrom)
            Plan.Swap(self.kl,kTo)
        elif not(PromotedFrom) and not(PromotedTo):
            Plan = PlanHandler(xi=np.abs(xi))\
    ...
    def SolveDX(self, xi=None,
                ...
                Show=False,
                SwapOnly=False, # Like in SX except can't work with doubles
                PlanOnly=False,
                **kwargs):
        ...
        if SwapOnly:
            Plan = PlanHandler(xi=np.abs(xi))\
                .Singlet(self.Nel, From=[self.kh, self,kh], To=[self.kl, self.kl])
            Plan.Swap(self.kh, kFrom)
            Plan.Swap(self.kl, kTo  )
            if Show: print(Plan)
